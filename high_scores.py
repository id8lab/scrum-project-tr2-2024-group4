...
def high_scores(screen):
    ...
    runner_up_image = pygame.image.load('runner_up.png')
    ...
    while running:
        ...
        runner_up_pos = (screen.get_width() // 2 - runner_up_image.get_width() // 2, 150)
        screen.blit(runner_up_image, runner_up_pos)
        ...
        for rank, score_entry in enumerate(high_scores_data[:10], start=1):
            try:
                if rank == 1:
                    position = (screen.get_width() // 2 - 75, 180)
                elif rank == 2:
                    position = (screen.get_width() // 2 - 175, 250)
                elif rank == 3:
                    position = (screen.get_width() // 2 + 75, 250)
                else:
                    position = (screen.get_width() // 2 - 100, y_offset)
                    y_offset += 40
