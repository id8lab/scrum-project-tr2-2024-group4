...
def high_scores(screen):
    ...
    while running:
        ...
        title_position = screen_width // 2 - title_font.size(title_text)[0] // 2
        x_offset = 0
        ...
        font_sizes = [60, 50, 40]
        y_offsets = [200, 300, 400]  # Different y positions for top 3 ranks to ensure they are separated properly
        for rank, score_entry in enumerate(high_scores_data[:3], start=1):
            try:
                ...
                position = (screen_width // 2 - score_text_rendered.get_width() // 2, y_offsets[rank-1])
